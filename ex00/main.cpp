...
int main()
{
    ...
    // テスト2: attack関数（エネルギーポイントとヒットポイントが十分にある場合）
    {
        ...
        // エネルギーポイントが1減っていることを確認 (C++98では直接確認できないので、必要に応じてデバッグ出力などを追加)
    }
	std::cout << std::endl;
    ...
    // テスト5: takeDamage関数
    {
        ...
        // ヒットポイントが5減っていることを確認 (C++98では直接確認できないので、必要に応じてデバッグ出力などを追加)
    ...
    // テスト6: beRepaired関数
    {
        ...
        // ヒットポイントが5増えていることを確認 (C++98では直接確認できないので、必要に応じてデバッグ出力などを追加)
    }
	std::cout << std::endl;
    ...
    // テスト7: beRepaired関数（エネルギーポイントが足りない場合）
    {
        ...
        clapTrap.setEnergyPoint(0);  // energyPointを直接変更(本来はprivateなので良くないが、テストのため)
        clapTrap.beRepaired(5);
    ...
    std::cout << "All tests passed!" << std::endl; // 全てのテストが正常終了したら出力
